...
function() {
	...
	location.href.indexOf("dev.web") < 0 ? angular.module("exceptionOverride", []).factory("$exceptionHandler", [function() {
	...
	}]) : angular.module("exceptionOverride", []), angular.module("Controllers").controller("appController", ["$rootScope", "$scope", "$timeout", "$log", "$state", "$window", "ngDialog", "mmpop", "appFactory", "loginFactory", "contactFactory", "accountFactory", "chatFactory", "confFactory", "contextMenuFactory", "notificationFactory", "utilFactory", "reportService", "actionTrack", "surviveCheckService", "subscribeMsgService", "stateManageService", function(e, t, o, n, r, a, i, c, s, l, u, f, d, g, m, p, h, M, y, C, S, v) {
		function w() {
		...
		function N(e) {
			...
			try {
				f.setSyncKey(e.SyncKey), f.setSyncCheckKey(e.SyncCheckKey), f.updateUserInfo(e.Profile, function() {}), angular.forEach(e.DelContactList, function(t) {
		...
		U.on("dragenter", function(e) {
			...
			k = t.target, t.dataTransfer.dropEffect = "none", U.addClass("drop-enter"), t.stopPropagation(), t.preventDefault()
		}), U.on("dragleave", function(e) {
			var t = e.originalEvent;
			t.dataTransfer.dropEffect = "none", k === t.target && U.removeClass("drop-enter"), t.stopPropagation(), t.preventDefault()
...
function() {
	...
	angular.module("Controllers").controller("loginController", ["$scope", "loginFactory", "utilFactory", "reportService", function(e, t, o, n) {
		$(".lang .lang-item").click(function(e) {
			...
			e.code = 0, 
			e.isScan = !1, e.isIPad = o.isIPad, e.isMacOS = o.isMacOS, e.isWindows = o.isWindows, e.lang = o.queryParser().lang || "zh_CN";
			var i = !1;
...
function() {
	...
	angular.module("Controllers").controller("contentChatController", ["$scope", "$timeout", "$state", "$log", "$document", "$compile", "chatFactory", "accountFactory", "contactFactory", "appFactory", "confFactory", "utilFactory", "chatroomFactory", "mmpop", "ngDialog", "preview", "reportService", "mmHttp", "emojiFactory", function(e, t, o, n, r, a, i, c, s, l, u, f, d, g, m, p, h, M, y) {
		...
		e.delState = !1, e.chatContent = [], e.isShowChatRoomMembers = !1, $(w).on("drop", function() {
		...
		e.$watchCollection("chatContent", function(e) {
			if (e.length > 0 && e[e.length - 1].FromUserName === c.getUserName()) E = !0;
				...
				E = w.scrollTop + w.clientHeight + 10 >= w.scrollHeight, P = !1
			}
...
function() {
	...
	angular.module("Controllers").controller("chatSenderController", ["$rootScope", "$scope", "$http", "$timeout", "ngDialog", "confFactory", "accountFactory", "contactFactory", "chatFactory", "screenShotFactory", "utilFactory", "mmpop", "stateManageService", "emojiFactory", "reportService", function(e, t, o, n, r, a, i, c, s, l, u, f, d, g, m) {
			...
			require.async(["webuploader"], function(e) {
				...
				try {
					B = e.create({
						...
						var c = Date.now();
						return B.md5File(e).then(function(t) {
							function n(e, t, o) {
							...
							if (r) {
								...
								f = angular.extend({}, d), o({
									...
									e.MMSendMsg && (e.MMSendMsg.MMFileStatus = a.MM_SEND_FILE_STATUS_FAIL, e.MMSendMsg.MMStatus = a.MSG_SEND_STATUS_FAIL), alert("上传失败")
								})
							} else f = angular.extend({}, d), n(e)
						}), !1
					}).on("fileQueued", function(e) {
						e.onQueued.call(e)
					}).on("uploadBeforeSend", function(e, t) {
						var o = e.file;
						o._data || {}, angular.extend(t, o._uploadParams, {
							uploadmediarequest: JSON.stringify(angular.extend({
								UploadType: 2
		...
		t.$on("$destroy", function() {
			...
			t && e != t && (x[t] = D.innerHTML), L = e, b(x[e])
			...
			l.isSupport() ? l.capture({
				ok: function() {
					...
					e = w(t), l.upload(JSON.stringify(t), function(t) {
		...
		t.editAreaKeyup = function(e) {
			...
			if (f.close(), !t.editAreaCtn.replace(/<br\/?>/g, "").match(/^\s*$/)) {
				...
				s.appendMessage(e), s.sendMessage(e), x[s.getCurrentUserName()] = "", t.editAreaCtn = ""
			}
			...
			W = {
				...
				onSuccess: function(e) {
					if (0 == e.BaseResponse.Ret) {
						...
						o.MediaId = e.MediaId, o.Signature = this.Signature, s.sendMessage(o), o.MMFileStatus = a.MM_SEND_FILE_STATUS_SUCCESS, t.$$phase || t.$digest()
...
function() {
	...
	angular.module("Controllers").controller("readMenuController", ["$rootScope", "$scope", "subscribeMsgService", function(e, t, o) {
		t.copyCallback = function() {
			...
			return console.log(o.current), t.closeThisMmPop(), o.current && o.current.Url
...
function() {
	...
	angular.module("Controllers").controller("transpondDialogController", ["$rootScope", "$scope", "$timeout", "$state", "$log", "$document", "chatFactory", "contactFactory", "appFactory", "chatroomFactory", "confFactory", "mmpop", "ngDialog", "utilFactory", "stateManageService", "accountFactory", function(e, t, o, n, r, a, i, c, s, l, u, f, d, g, m, p) {
		function h(e, t) {
			...
			o.ToUserName = t, o.FromUserName = p.getUserName(), o.isTranspond = !0, o._h = void 0, o._offsetTop = void 0, o.MMSourceMsgId = e.MsgId, o.Content = o.MMActualContent, o.Scene = 2, o = i.createMessage(o), i.appendMessage(o), i.sendMessage(o)
		}
			...
			_sendFileUserName, service = {
				...
				_sendCheck: function(e) {
					...
					if (t && o) {
						...
						return a && i || utilFactory.reportSendState("sendcheckAttrError"), e || utilFactory.reportSendState("toUserNameNotFound"), a == i && i == e ? !0 : (i != e && utilFactory.reportSendState("toUserNameConflictNav"), a != e && utilFactory.reportSendState("toUserNameConflictChat"), utilFactory.reportSendState("uiCheckFail"), reportService.report(reportService.ReportType.sendError, {
							type: "uiCheckFail",
...
function() {
	...
	angular.module("Services").factory("accountFactory", ["$q", "confFactory", "utilFactory", function(e, t, o) {
		var n, r, a, i = {},
			...
			d = {
				openNotify: function() {
					u = t.MM_NOTIFY_OPEN, o.setCookie("MM_WX_NOTIFY_STATE", t.MM_NOTIFY_OPEN)
				},
				closeNotify: function() {
					u = t.MM_NOTIFY_CLOSE, o.setCookie("MM_WX_NOTIFY_STATE", t.MM_NOTIFY_CLOSE)
				},
				isNotifyOpen: function() {
					return !!u
				},
				openSound: function() {
					f = t.MM_SOUND_OPEN, o.setCookie("MM_WX_SOUND_STATE", t.MM_SOUND_OPEN)
				},
				closeSound: function() {
					f = t.MM_SOUND_CLOSE, o.setCookie("MM_WX_SOUND_STATE", t.MM_SOUND_CLOSE)
				},
		...
		return u == t.MM_NOTIFY_OPEN ? d.openNotify() : d.closeNotify(), f == t.MM_SOUND_OPEN ? d.openSound() : d.closeSound(), d
	}])
}(), !
function() {
	"use strict";
		...
		var d, g = window._contacts = {},
			...
			P = {
				...
				getMemberDisplayName: function(e) {
					return I.getChatroomIdByUserName(e), e && C[e] ? C[e][this.UserName] : ""
				},
			...
			I = {
				...
				addBatchgetContact: function(e, t, r) {
					function i(e) {
						...
						}), console.timeEnd("addContactsHandler"), d.addContacts(e.ContactList, !0), v = [], !v.length && S.length > 0 && d.batchGetContact().then(i, s)
					}
					function s(e) {
						var t = v;
						...
						}), v.length || !S.length && !w.length || d.batchGetContact().then(i, s)
					}
					var l, u, f = o.defer(),
						d = this;
					if (e && e.UserName) {
				...
				n.bind("paste", function() {
					...
					s && clearTimeout(s), s = setTimeout(function() {
						...
						if (-1 != l && -1 != u) {
							...
							w && (document.createRange ? (S = document.createRange(), S.setStartAfter(w), S.collapse(!1), v = window.getSelection(), v.removeAllRanges(), v.addRange(S)) : document.selection && (S = document.body.createTextRange(), S.moveToElementText(w), S.collapse(!1), S.select()), w.parentNode.removeChild(w)), a.$setViewValue(p + _ + h), s = null
						}
					}, 50)
			...
			var _ = {
				...
				S = {
					open: function(a) {
						...
						return g.when(c(v.template || v.templateUrl)).then(function(t) {
							if (f.put(v.template || v.templateUrl, t), v.showClose && (t += '<div class="ngdialog-close"></div>'), u.$result = b = o('<div id="ngdialog' + r + '" class="ngdialog"></div>'), b.html(v.overlay ? '<div class="ngdialog-overlay"></div><div class="ngdialog-content">' + t + "</div>" : '<div class="ngdialog-content">' + t + "</div>"), v.data && e.isString(v.data)) {
							...
							if (E.closeThisDialog = function(e) {
								...
								var t = v.closeByDocument ? o(e.target).hasClass("ngdialog-overlay") : !1,
